      ...
      <div class="card">
        ...
        <div class="flex items-center gap-2 mb-4">
          <Button
            ...
            @click="goBack"
            class="p-button-sm"
            v-tooltip.top="'Volver al banco de preguntas'"
          />
        ...
        <p class="text-muted-color mb-6">Crea una nueva pregunta y asígnala a un examen</p>
            ...
            <div class="col-12">
              <div class="field">
                ...
                <Select
                  ...
                  class="w-full"
                  :class="{ 'p-invalid': errors.exam_id }"
                  :loading="isLoadingExams"
                  filter
                  filterPlaceholder="Buscar examen..."
                >
                  <template #option="slotProps">
                    <div class="flex items-center justify-between w-full">
                      <div>
                        <div class="font-medium">{{ slotProps.option.title }}</div>
                        <div class="text-sm text-muted-color" v-if="slotProps.option.description">
                          {{ slotProps.option.description.substring(0, 60)
                          }}{{ slotProps.option.description.length > 60 ? '...' : '' }}
                        </div>
                      </div>
                    </div>
                  </template>
                </Select>
                <small v-if="errors.exam_id" class="p-error">{{ errors.exam_id }}</small>
                <small class="text-muted-color mt-2 block">
                  Selecciona el examen al que pertenecerá esta pregunta
                </small>
              </div>
            </div>
...
import type { CreateQuestionRequest, Exam } from '@/types'
...
const loadExams = async () => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.message || 'Error al cargar los exámenes',
      life: 5000,
    })
  } finally {
...
const validateForm = (): boolean => {
  ...
  if (!formData.exam_id || formData.exam_id === 0) {
    errors.value.exam_id = 'Debes seleccionar un examen'
    isValid = false
  }
