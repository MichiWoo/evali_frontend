  ...
  <div class="grid">
    <div class="col-12">
      <div class="card">
        ...
        <!-- Loading State -->
        <div v-if="isLoadingQuestion" class="flex justify-center items-center py-8">
          <ProgressSpinner />
        </div>

        <!-- Error State -->
        <Message v-if="questionError" severity="error" :closable="false" class="mb-4">
          {{ questionError }}
        </Message>

        <!-- Form -->
        <form v-if="question && !isLoadingQuestion" @submit.prevent="handleSubmit">
          <div class="grid">
            <!-- Question Text -->
            <div class="col-12">
              <div class="field">
...
const question = ref(questionStore.currentQuestion)
...
// Form Data
const formData = reactive<UpdateQuestionRequest>({
  text: '',
...
const loadQuestion = async () => {
  try {
    ...
    question.value = questionStore.currentQuestion

    if (!question.value) {
      questionError.value = 'Pregunta no encontrada'
      return
    }

    // Populate form with existing data
    formData.text = question.value.text || ''
    formData.type = question.value.type || 'multiple_choice'
    formData.points = question.value.points || 1

    // Populate meta
    if (question.value.meta) {
      formData.meta = {
        difficulty: question.value.meta.difficulty,
        hint: question.value.meta.hint,
        explanation: question.value.meta.explanation,
        correct_answer: question.value.meta.correct_answer,
        options: [],
      }
    }
    ...
    // Set correct_answer for true/false if it exists
    if (question.value.type === 'true_false' && question.value.meta) {
      if ('correct_answer' in question.value.meta) {
        formData.meta = {
          ...formData.meta,
          correct_answer: question.value.meta.correct_answer,
        }
    ...
    questionError.value = err.message || 'Error al cargar la pregunta'
...
const onTypeChange = () => {
  ...
  if (formData.type === 'multiple_choice') {
    ...
    // Keep existing correct_answer if it exists
    if (
      formData.meta?.correct_answer === undefined &&
      question.value?.meta?.correct_answer !== undefined
    ) {
      formData.meta = {
        ...formData.meta,
        correct_answer: question.value.meta.correct_answer,
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Redirect back to question detail
